...
CylinderPtr Cylinder::Make (int nstack, int nslice)
{
  return CylinderPtr(new Cylinder(nstack,nslice));
}

Cylinder::Cylinder (int nstack, int nslice)
{
  GridPtr grid = Grid::Make(nstack,nslice);
  m_nind = grid->IndexCount();

  m_topdisk = Disk3D::Make();
  m_botdisk = Disk3D::Make();
  ...
  for (int i=0; i<2*grid->VertexCount(); i+=2) {
    ...
    coord[nc+0] = sin(-theta);  //z
    coord[nc+1] = -phi; //y
    coord[nc+2] = cos(-theta); //x
